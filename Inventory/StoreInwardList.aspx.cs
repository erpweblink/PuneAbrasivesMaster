...
public partial class Admin_StoreInwardList : System.Web.UI.Page
{
    ...
    public static List<string> AutoFillCustomerName(string prefixText)
    {
        using (SqlConnection con = new SqlConnection())
        {
            ...
            {
                com.CommandText = "SELECT DISTINCT [Suppliername] FROM [tbl_PendingInwardHdr] where " + "Suppliername like @Search + '%' and IsDeleted=0";
    ...
    protected void txtCustomerName_TextChanged(object sender, EventArgs e)
    {
        if (txtCustomerName.Text != "" || txtCustomerName.Text != null)
        {
            ...
            SqlDataAdapter sad = new SqlDataAdapter("select * from [tbl_PendingInwardHdr] where Suppliername = '" + company + "' AND IsDeleted = 0", Cls_Main.Conn);
            sad.Fill(dt);
    ...
    {
        Response.Redirect("StoreInwardEntry.aspx");
    }
    ...
    protected void GVInward_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("StoreInwardEntry.aspx?Id=" + objcls.encrypt(e.CommandArgument.ToString()) + "");
        }

        if (e.CommandName == "RowDelete")
        {
            Cls_Main.Conn_Open();
            SqlCommand Cmd = new SqlCommand("UPDATE [tbl_PendingInwardHdr] SET IsDeleted=@IsDeleted,DeletedBy=@DeletedBy,DeletedOn=@DeletedOn WHERE ID=@ID", Cls_Main.Conn);
            ...
            con.Open();
            SqlCommand cmd1 = new SqlCommand("select OrderNo from tbl_PendingInwardHdr where Id = " + Convert.ToInt32(e.CommandArgument.ToString()) + "", con);
    ...
    protected void GVInward_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM tblUserRoleAuthorization where UserID = '" + id + "' AND PageName = 'StoreInwardList.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
        ...
        try
        {
            ...
            else
            {
                if (txtOrderno.Text != "")
                {
                    ...
                    DataTable dt = new DataTable();
                    SqlDataAdapter sad = new SqlDataAdapter("select * from [tbl_PendingInwardHdr] where OrderNo = '" + OrderNo + "' AND IsDeleted = 0", Cls_Main.Conn);
                    sad.Fill(dt);
                    GVInward.EmptyDataText = "Not Records Found";
                    GVInward.DataSource = dt;
                    GVInward.DataBind();
                ...
                if (!string.IsNullOrEmpty(txtfromdate.Text) && !string.IsNullOrEmpty(txttodate.Text))
                {
                    ...
                    SqlDataAdapter sad = new SqlDataAdapter(" SELECT * FROM [tbl_PendingInwardHdr] WHERE CreatedOn between'" + txtfromdate.Text + "' AND '" + txttodate.Text + "' ", Cls_Main.Conn);
                    sad.Fill(dt);
    ...
    public static List<string> AutoFillOrderNo(string prefixText)
    {
        using (SqlConnection con = new SqlConnection())
        {
            ...
            using (SqlCommand com = new SqlCommand())
            {
                com.CommandText = "SELECT [ID],[InvoiceNo] FROM [tbl_PendingInwardHdr] where " + "InvoiceNo like '%'+ @Search + '%' and IsDeleted=0";
    ...
    protected void txtOrderno_TextChanged(object sender, EventArgs e)
    {
        ...
        DataTable dt = new DataTable();
        SqlDataAdapter sad = new SqlDataAdapter("select * from [tbl_PendingInwardHdr] where InvoiceNo = '" + OrderNo + "' AND IsDeleted = 0", Cls_Main.Conn);
        sad.Fill(dt);
        GVInward.EmptyDataText = "Not Records Found";
        GVInward.DataSource = dt;
        GVInward.DataBind();
